...
from src.pipeline.brand_impersonation import BrandImpersonationDetector
...
class TestBrandImpersonation:
    ...
    def run_all_tests(self):
        ...
        print(
            "TEST CATEGORY 2: PHISHING URLS WITH BRAND IMPERSONATION (Should trigger)"
        )
        print("-" * 100)
    ...
    def _print_results(self):
        ...
        for result in self.test_results:
            ...
            print(
                f"   Expected: {result['expected']} | Actual: {result['actual']} | Risk Score: {result['risk_score']}"
            )
            ...
            if result["reasons"]:
                print(
                    f"   Reasons: {', '.join(result['reasons'][:2])}"
